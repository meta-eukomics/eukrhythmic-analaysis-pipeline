...
snakemake  \
    -s $SNAKEFILE $RERUN_INCOMPLETE --jobs $JOBS --conda-frontend mamba --use-conda --cluster-config cluster.yaml --rerun-incomplete --cluster "sbatch --parsable --qos=unlim --partition={cluster.queue} --job-name=${jobname}.{rule}.{wildcards} --mem={cluster.mem}gb --time={cluster.time} --ntasks={cluster.tasks} --nodes={cluster.nodes} --cpus-per-task={cluster.cpupertask} --requeue"
